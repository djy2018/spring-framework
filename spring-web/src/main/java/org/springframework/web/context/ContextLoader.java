...
public class ContextLoader {
	 ...
	 * default otherwise.
	 *
	 * servlet上下文参数的名称(即， {@value})来指定根上下文的配置位置，否则返回到实现的默认值。
	 *
	 ...
	 * Root WebApplicationContext 实现类的配置参数
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		...
		// 则抛出 IllegalStateException 异常。例如，在 web.xml 中存在多个 ContextLoader 。
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) { // <4.1> 未刷新(激活)
					...
					if (cwac.getParent() == null) { // <4.2> 无父容器
			...
			// <6> 记录到 currentContext 或者 currentContextPerThread 中
			...
			// <7> info级别，打印初始化 Root WebApplicationContext 耗时
			if (logger.isInfoEnabled()) {
			...
			// <8> 发生异常，记录异常到 WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE 中
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		if (contextClassName != null) { // 情况一，如果值非空，则获得该类
			try {
		...
		else {  // 情况二，从 defaultStrategies 获得该类
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			...
			if (idParam != null) {        // 情况一：使用 contextId 属性
				wac.setId(idParam);
			}
			else { // 情况二：自动生成
				// Generate default id...
		...
		// 当上下文被刷新时，wac环境的#initPropertySources在任何情况下都会被调用;
		// 在这里确保servlet属性源已经就绪，可以用于#refresh之前发生的任何后处理或初始化
	...
	public void closeWebApplicationContext(ServletContext servletContext) {
		...
		try {
			...
			// 移除 	currentContext 和 currentContextPerThread
			ClassLoader ccl = Thread.currentThread().getContextClassLoader();
			if (ccl == ContextLoader.class.getClassLoader()) {
				currentContext = null;
			}
			...
			// 移除上下文属性 WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE
