...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		// 初始化 MultipartResolver - 内容类型( Content-Type )为 multipart/* 的请求的解析器接口
		initMultipartResolver(context);
		// 初始化 LocaleResolver - 本地(国际化)解析器
		initLocaleResolver(context);
		// 初始化 ThemeResolver - 主题解析器接口
		initThemeResolver(context);
		// 初始化 HandlerMapping - 处理器匹配接口，根据请求获得其处理器( handler )和拦截器们( HandlerInterceptor 数组 )
	 ...
	 * <p>
	 * 为给定的策略接口创建一个默认策略对象列表。默认实现使用 "DispatcherServlet.properties"文件
	 * (与 DispatcherServlet 类在同一个包中)来确定类名。它通过上下文的 BeanFactory 实例化策略对象。
	 *
	 * @param context           the current WebApplicationContext
	 * @param strategyInterface the strategy interface
	 ...
	 * <p>
	 * 处理处理器选择和处理程序调用的结果，该结果要么是ModelAndView，要么是要解析为ModelAndView的异常
	 */
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
									   @Nullable HandlerExecutionChain mappedHandler, @Nullable ModelAndView mv,
									   @Nullable Exception exception) throws Exception {
		// <1> 标记，是否是异常转换生成的 ModelAndView 对象
